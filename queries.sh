...
# querying for the winners goals is enough, as it is impossible for the losing
# team of a match to be the one with most goals in a tournament
...
echo "$($PSQL "SELECT teams.name FROM teams INNER JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
